...
| Inbox apply applies a message's **local effect exactly once** under concurrent drainers: rows are claimed with `FOR UPDATE SKIP LOCKED`, so the effect is never double-applied. *Known gap (found by audit, fix pending):* the post-rollback **failure-recording** path runs in a separate, unlocked transaction and can write a spurious `retrying`/dead-letter status + metric if another drainer succeeds in between — the effect stays exactly-once, but the status/counter can lie. See [inbox stale-failure write race](docs/09-lessons-learned/inbox-stale-failure-write-race.md). Multi-instance safety is consumer-side only; the outbox has no publisher claim (see boundaries below). | [`NotificationsInboxProcessor.cs`](src/Modules/Notifications/ModulithReliabilityKit.Modules.Notifications.Infrastructure/Processing/NotificationsInboxProcessor.cs) | [`InboxConcurrencyReliabilityTests`](src/Tests/ModulithReliabilityKit.IntegrationTests/Notifications/InboxConcurrencyReliabilityTests.cs) · `A_Row_Claimed_By_One_Processor_Is_Skipped_By_A_Concurrent_Processor` |
...
- **Exactly-once *local database* apply** — the inbox dispatcher effect and the `processed` mark commit
  ...
  covered** — the shipped dispatcher only writes to the same database. *Known gap:* the post-failure
  bookkeeping (`RecordFailureAsync`) runs in a separate, unlocked transaction and can record a spurious
  `retrying`/dead-letter status + metric if another drainer succeeds in between; the effect stays
  exactly-once but the status/counter can lie. Fix is test-first — see
  [inbox stale-failure write race](docs/09-lessons-learned/inbox-stale-failure-write-race.md).
