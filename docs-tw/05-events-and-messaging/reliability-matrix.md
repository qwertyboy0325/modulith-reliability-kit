...
6. **鎖只保護它包住的臨界區。** inbox 套用以 `FOR UPDATE SKIP LOCKED` claim;已 rollback 的 drainer 的*失敗記錄*(`RecordFailureAsync`)跑在後續的交易,因此它也必須重新 claim 該列(阻塞式 `FOR UPDATE`)並在已被處理時 no-op —— 否則會把假的重試／死信覆蓋到另一個 drainer 的併發成功上。此過期寫入競態由 claim 稽核發現並以 test-first 修正;可重現的失敗與修正見 `09-lessons-learned/inbox-stale-failure-write-race.md`。
