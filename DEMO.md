...
| `InboxConcurrencyReliabilityTests` | **Multi-instance safe**: `FOR UPDATE SKIP LOCKED` ⇒ a concurrent drainer skips a claimed row (effect applied once), and failure-recording re-claims under `FOR UPDATE` so a rolled-back drainer can't overwrite a concurrent success — regression for the stale-write race in [`docs/09-lessons-learned/inbox-stale-failure-write-race.md`](docs/09-lessons-learned/inbox-stale-failure-write-race.md) |
